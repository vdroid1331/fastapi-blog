...
6. Apply the database migrations by running `uv run alembic upgrade head`.
7. Run fastapi server in dev mode using `uv run fastapi dev`.
8. (Optional) Run the populate_db.py script after it by running `uv run python populate_db.py`.
